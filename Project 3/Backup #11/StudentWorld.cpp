...
void StudentWorld::addNewShip()
{
	...
	if (currAliens < compareMin(maxAliens, remainingShips))
	{
		...
		if (randInt(0, 100) < 10)
		{
			Objects.push_back(new Smallgon(VIEW_WIDTH - 1, randInt(0, VIEW_HEIGHT - 1), this));
			//Objects.push_back(new Smoregon(VIEW_WIDTH - 1, randInt(0, VIEW_HEIGHT - 1), this));
			//Objects.push_back(new Snagglegon(VIEW_WIDTH - 1, randInt(0, VIEW_HEIGHT - 1), this));
		//	currAliens++;
		}
	}
}
...
double StudentWorld::eucl_dist(double x1, double y1, double x2, double y2)
{
...
/*bool StudentWorld::collideAllPossible(Actor* mainObject)
{
	...
	for (T; T != Objects.end(); ++T)
	{
			...
			if (doesCollide(mainObject, *T) && canCollide(mainObject, *T))
			{
				mainObject->sufferDamage((*T)->ID());
				(*T)->sufferDamage(mainObject->ID());
				return true;
			}
			else if (doesCollide(mainObject, Nachen) && canCollide(mainObject, Nachen))
			{
				mainObject->sufferDamage((Nachen)->ID());
				Nachen->sufferDamage(mainObject->ID());
				return true;
			}
